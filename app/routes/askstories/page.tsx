...
export default function TopStories() {
  const [askstories, setaskstories] = useState<StoriesProps[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const askstories: StoriesProps[] = await Promise.all(storyPromises);
      setaskstories(askstories);
      setFilteredStories(askstories);
      setLoading(false);
    };
    fetchData();
  }, []);
  ...
  useEffect(() => {
    const result = askstories.filter((story) =>
    ...
    <div className="container mx-auto min-w-8 p-4">
      <div className="py-3">
        <h1 className="text-3xl font-bold flex justify-center pb-4">
          Ask <span className="text-[#423D8A]">Stories</span>
        </h1>
